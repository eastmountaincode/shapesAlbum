...
There were minimal changes to the previous iteration of the model. stretchShape methods now take in an int as a new width or height, and they have been refactored to reflect the revelation that these are basically just setter methods. Many, if not all, doubles in the model were changed to ints for simplicity. 
